...
from cat._source._srw_utils import _undulator
from cat._source._srw_utils import _srw_electron_beam
from cat._source._srw_utils import _propagate_wave_front
...
def _cal_wfrs(
    ...
    vir_wavefront = np.reshape(vir_wavefront.wfr.arEx, (n_row, n_column, 2))
        ...
        for i_part in e_crank_count:
            ...
            for ie in range(i_part):
                ...
                if method is 'srw':
                    ...
                    wfr_arex = np.reshape(
                        np.array(wavefront.wfr.arEx), (n_row, n_column, 2)
                        )
                    ...
                    shift_wfr = np.zeros((n_row, n_column), dtype = np.complex128)
